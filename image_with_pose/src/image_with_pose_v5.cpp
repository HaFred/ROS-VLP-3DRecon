/* v5 the final version of fusing positioning from tf_echo odom -> map (which is in /tf, back and forth processed by the loosely coupled EKF), and the rotation from odom itself (base_footprint -> odom).
...
#define VERSION_NUMBER "5"  // programmed by fred
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg) // these msg are setup to receive msg from the topic which is encoded by the publisher 
{
    ...
    try{
        ...
        ROS_ASSERT( cv::imwrite( std::string(SAVE_PATH) + std::string( "image_at_" ) + current_time_stamp + std::string( ".jpg" ), cv_ptr->image ) );
